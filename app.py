...
def generate_report():
    try:
        ...
        all_df_columns = ["Item ID", "Item Name"] + [c for c in columns if c not in ["Item ID", "Item Name"]]
        df_data = pd.DataFrame(rows).reindex(columns=all_df_columns)
        ...
        for col in ['Group Status', 'Potential', 'Dept', 'Country/Region', 'Salesperson', 'Referral Source Category']:
            if col not in df_data.columns: df_data[col] = "N/A"
        ...
        for t in ["Addition", "Removal"]:
            ...
            write_merged_title(ws_summary, title, align="left", col_span=len(cols)); ws_summary.append([])
            ws_summary.append(cols); s = ws_summary.max_row
            if not df_s.empty:
                for _, r in df_s[cols].iterrows(): ws_summary.append(list(r.fillna('')))
        ...
        if not df_active.empty:
            ...
            if not matrix.empty:
                ...
                write_merged_title(ws_summary, "Active Enquiries by Country and Potential", align="left", col_span=final_matrix.shape[1]); ws_summary.append([])
            ...
            if not pivot.empty:
                ...
                write_merged_title(ws_summary, "Active Enquiries by Market Division", align="left", col_span=final_matrix.shape[1]); ws_summary.append([])
                for r in dataframe_to_rows(final_matrix, index=False, header=True): ws_summary.append(r)
                style_last_written_table(ws_summary, "Active Enquiries by Market Division", bold_cols=["Total"]); ws_summary.append([])
        ...
        if not effectiveness.empty:
            ...
            grand_total_data = {"Referral Source Category": "Grand Total", "Total": effectiveness["Total"].sum(), "Won": effectiveness["Won"].sum()}
            grand_total_data["Win %"] = str(round((grand_total_data["Won"] / grand_total_data["Total"] if grand_total_data["Total"] > 0 else 0) * 100, 1)) + "%"
            effectiveness = pd.concat([effectiveness, pd.DataFrame([grand_total_data])], ignore_index=True)
            write_merged_title(ws_summary, "Referral Source Effectiveness", align="left", col_span=effectiveness.shape[1]); ws_summary.append([])
            for r in dataframe_to_rows(effectiveness, index=False, header=True): ws_summary.append(r)
            style_last_written_table(ws_summary, "Referral Source Effectiveness", bold_cols=["Win %"]); ws_summary.append([])
        
        # --- FULL DEPARTMENT, DESK, AND SALESPERSON SECTIONS ---
        section_cols = ["Item Name", "Country/Region", "Salesperson", "Service", "Stage", "Potential", "Referral Source Category"]
        
        ws_summary.append([]); write_merged_title(ws_summary, "Breakdown by Departments", col_span=len(section_cols), align="center"); ws_summary.append([])
        depts_to_report = ["COS", "CCT-GBA", "CCT-SH", "AG2", "TAX"]
        for dept in depts_to_report:
            df_dept = df_active[df_active["Dept"].fillna("").str.contains(dept, case=False, na=False)]
            ...
            if not df_dept.empty:
                for _, r in df_dept[section_cols].iterrows(): ws_summary.append(list(r.fillna('')))
            format_table(ws_summary, s, 1, 1+len(df_dept), len(section_cols), align="left"); ws_summary.append([])

        ws_summary.append([]); write_merged_title(ws_summary, "Individual Desks", col_span=len(section_cols), align="center"); ws_summary.append([])
        if "Market Division" not in df_active.columns: df_active["Market Division"] = df_active["Country/Region"].apply(map_market)
        ...
        for desk in desks_to_report:
            ...
            if not df_desk.empty:
                for _, r in df_desk[section_cols].iterrows(): ws_summary.append(list(r.fillna('')))
            format_table(ws_summary, s, 1, 1+len(df_desk), len(section_cols), align="left"); ws_summary.append([])
            
        ws_summary.append([]); write_merged_title(ws_summary, "Breakdown by Salesperson", col_span=5, align="center"); ws_summary.append([])
        ...
        if not df_sales.empty:
            for salesperson, group in df_sales.groupby("Salesperson"):
                ...
                for _, r in group.iterrows():
                    ws_summary.append([""] + [r.get(c, '') for c in sales_cols])
